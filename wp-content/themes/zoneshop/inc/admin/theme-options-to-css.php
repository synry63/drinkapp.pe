...
function customcss() {
	...
	if ( isset( $thim_options['thim_footer_bg_image'] ) && $thim_options['thim_footer_bg_image'] <> '' ) {
		$footer_bg_image = wp_get_attachment_image_src( $thim_options['thim_footer_bg_image'], 'full' );
		$custom_css .= ' .site-footer{background-image: url("' . $footer_bg_image[0] . '");}';
	}
	...
	if ( $thim_options['thim_bg_border_header_color'] <> '' ) {
		$custom_css .= '.site-header .navigation.affix-top{border-bottom: 2px solid ' . $thim_options['thim_bg_border_header_color'] . ' }';
	}
	...
	// Header style 3
	$custom_css .= '
		.header_v3 .wrapper-logo .table-cell {
			width:         ' . $width_banner . '%;
		}
		.header_v3 .wrapper-logo .sm-logo {
			width:      ' . $width_logo . '%;
		}
	';
