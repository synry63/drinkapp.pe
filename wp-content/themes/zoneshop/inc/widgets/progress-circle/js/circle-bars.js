...
jQuery('.progress-circle').waypoint(function () {

	var $this = jQuery(this),
			bar = $this.find('.progress-single-circle'),
			units = bar.data('units'),
			color = bar.data('color'),
			size = bar.data('size'),
			val =  bar.data('progress-value') / 100,
			val_progess =  bar.data('progress-value');
    	jQuery(this).find('.progress-single-circle').circleProgress ({
			value: val,
			size: size,
			thickness: 3,
			startAngle: -8,
			...
			jQuery(this).find('strong').html(parseInt(val_progess * progress) + '<i>' + units + '</i>');
		});
 }, { offset:'85%' });
