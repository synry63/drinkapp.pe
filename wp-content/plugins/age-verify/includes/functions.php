...
function av_get_verify_form() {
	...
	$submit_button_label = apply_filters( 'av_form_submit_label', __( 'Empecemos &raquo;', 'age_verify' ) );
	...
	if ( $error ) :
		...
		// Catch-all error
		$error_string = apply_filters( 'av_error_text_general', __( 'LO SENTIMOS, PERO ALGO DEBE HABER SALIDO MAL. POR FAVOR, INTÉNTALO DE NUEVO', 'age_verify' ) );
		...
		if ( $error == 3 )
			$error_string = apply_filters( 'av_error_text_too_young', __( 'NO PUEDES ACCEDER A UNA EXPERIENCIA CON DRINKAPP NECESITAS SER MAYOR DE EDAD PARA HACERLO', 'age_verify' ) );
		
		// Visitor entered an invalid date
		if ( $error == 4 )
			$error_string = apply_filters( 'av_error_text_bad_date', __( 'POR FAVOR INTRODUZCA UNA FECHA VALIDA', 'age_verify' ) );
	...
	switch ( $input_type ) {
		...
		case 'inputs' :
			...
			$form .= '<div class="dates-age-wrap">
			<p><label for="av_verify_d">Día</label><input type="text" name="av_verify_d" id="av_verify_d" maxlength="2" value="" placeholder="DD" /></p>
			<p><label for="av_verify_m">Mes</label><input type="text" name="av_verify_m" id="av_verify_m" maxlength="2" value="" placeholder="MM" /></p>
			<p><label for="av_verify_y">Año</label><input type="text" name="av_verify_y" id="av_verify_y" maxlength="4" value="" placeholder="YYYY" /></p>
			</div>';
    ...
    $form .= '<div class="submit checkboxFive">
    <input type="checkbox" name="av_verify_remember" id="av_verify_remember" value="1" /><label for="av_verify_remember"></label><span>' . esc_html__( 'Recordarme', 'age_verify' ) . '</span></div>';
	$form .= '<input class="btn-style" type="submit" name="av_verify" id="av_verify" value="' . esc_attr( $submit_button_label ) . '" />';
